...
async def get_messages(match_id: str, user_id: str, db: AsyncSession) -> list[Message]:
...
async def save_message(match_id: str, sender_id: str, content: str, db: AsyncSession) -> Message:
...
async def mark_read(match_id: str, user_id: str, db: AsyncSession):
    await db.execute(
        update(Message)
        .where(
            and_(
